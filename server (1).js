...
function mobileKeyMiddleware(req, res, next) {
    ...
    // ⚠️ DUİZELTME: Content-Type sadece /api/ rotalarında JSON olarak set edilir.
    // / veya /chatsee gibi sayfa isteklerinde önceden 'application/json' set edilirse
    // sendFile HTML’i ham metin olarak gönderir — tarayıcı HTML render etmez.
...
app.get('/chatsee', (req, res) => {
    ...
    fssync.existsSync(p) ? res.sendFile(p) : res.status(404).send('ChatSee: public/chatsee/index.html bulunamadı');
});
app.get('/chatsee/*', (req, res) => {
    const p = path.join(__dirname, 'public', 'chatsee', 'index.html');
    fssync.existsSync(p) ? res.sendFile(p) : res.status(404).send('ChatSee: public/chatsee/index.html bulunamadı');
