...
app.post('/api/messages', authenticateToken, checkRestriction('message'), async (req, res) => {
    try {
        ...
        res.status(201).json({ message: 'Mesaj gönderildi', messageId: msgId, id: msgId });
    } catch (error) {
...
// ─── 42b. POST → HİKAYEYE PAYLAŞ ──────────────────────────────────
// POST /api/stories/share/:postId
// Bir gönderiyi hikayeye ekler: post'un medya URL'sini veya text içeriğini
// 24 saatlik hikaye olarak kaydeder
app.post('/api/stories/share/:postId', authenticateToken, async (req, res) => {
    try {
        const { postId } = req.params;
        const post = await dbGet(
            `SELECT p.*, u.username, u.name FROM posts p JOIN users u ON p."userId" = u.id WHERE p.id = $1 AND p."isActive" = TRUE`,
            [postId]
        );
        if (!post) return res.status(404).json({ error: 'Gönderi bulunamadı' });

        // Medya varsa görsel hikaye, yoksa metin hikayesi oluştur
        const mediaUrl  = post.mediaUrl || post.media || null;
        const mediaType = mediaUrl ? (mediaUrl.match(/\.(mp4|webm|mov)$/i) ? 'video' : 'image') : 'text';
        const caption   = post.content ? post.content.substring(0, 200) : null;
        const text      = !mediaUrl ? (post.content ? post.content.substring(0, 300) : 'Paylaşılan gönderi') : null;

        const storyId = uuidv4();
        await dbRun(
            `INSERT INTO stories (id, "userId", "mediaUrl", "mediaType", caption, text, "textColor", "createdAt", "expiresAt")
             VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW() + INTERVAL '24 hours')`,
            [storyId, req.user.id, mediaUrl || '', mediaType, caption, text, '#FFFFFF']
        );

        // Post sahibine bildirim gönder (kendisi değilse)
        if (post.userId !== req.user.id) {
            await createNotification(post.userId, 'story_share', `${req.user.username} gönderinizi hikayesine ekledi`, { storyId, postId }).catch(() => {});
        }

        res.status(201).json({ message: 'Gönderi hikayeye eklendi', storyId });
    } catch (error) {
        console.error('Story paylaşım hatası:', error);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});
...
// ─── AI CHAT: /api/ai-chat ──────────────────────────────────────────
// Öncelik: GROQ → OPENAI → ANTHROPIC → llama.cpp
// Streaming (SSE) destekli — kelime kelime gerçek zamanlı yanıt
// Tarımsal + hava durumu sorgularında otomatik weather API entegrasyonu
// =======================================================================
...
// ── Hava durumu gerektiren kelime kalıpları ──────────────────────────
const WEATHER_KEYWORDS = [
    'hava', 'yağmur', 'kar', 'sıcaklık', 'don', 'nem', 'rüzgar', 'fırtına',
    'ekim', 'dikim', 'ekilir mi', 'dikilir mi', 'ekmeli miyim', 'dikebilir miyim',
    'bugün', 'yarın', 'bu hafta', 'mevsim', 'hasat', 'sulama', 'don riski',
    'bitki eker', 'tohum', 'bitki diker', 'tarla', 'bahçe', 'sera', 'aşı',
    'gübreleme', 'ilaçlama', 'çıkış yapayım', 'ekebilir miyim'
];

function needsWeather(messages) {
...
// ── Hava durumu verisini al (kendi /api/weather/current endpoint'ini kullan) ──
async function fetchWeatherForAI(lat, lon, city) {
    try {
        ...
        let url;
        if (lat && lon) {
            url = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apiKey}&lang=tr&units=metric`;
        } else if (city) {
            url = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&appid=${apiKey}&lang=tr&units=metric`;
        } else {
            return null;
        }
        ...
        return {
            sehir: d.name,
            sicaklik: Math.round(d.main?.temp),
            hissedilen: Math.round(d.main?.feels_like),
            nem: d.main?.humidity,
        ...
        console.warn('[AI Weather] Hava durumu alınamadı:', e.message);
        return null;
    }
}

// ── Sağlayıcı bazlı AI isteği (streaming + non-streaming) ───────────
async function callAIProvider(messages, stream, max_tokens, temperature, res) {
    const { default: fetch } = await import('node-fetch');
    const maxTok = Math.min(parseInt(max_tokens) || 1024, 4096);
    const temp   = Math.min(parseFloat(temperature) || 0.7, 2.0);

    // ── 1. GROQ (en hızlı, ücretsiz) ────────────────────────────────
        ...
        try {
            const r = await fetch('https://api.groq.com/openai/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages, stream: !!stream, max_tokens: maxTok, temperature: temp
                }),
                signal: AbortSignal.timeout(AI_TIMEOUT_MS)
            });
            if (!r.ok) throw new Error(`Groq HTTP ${r.status}`);
            console.log('[AI] Groq sağlayıcısı kullanıldı');
            return r;
        } catch (e) { console.warn('[AI] Groq başarısız, sıradaki sağlayıcıya geçiliyor:', e.message); }
    }
    ...
    if (process.env.OPENAI_API_KEY) {
        try {
            ...
            console.log('[AI] OpenAI sağlayıcısı kullanıldı');
            return r;
        } catch (e) { console.warn('[AI] OpenAI başarısız, sıradaki sağlayıcıya geçiliyor:', e.message); }
    }

    // ── 3. ANTHROPIC (stream=false fallback — Anthropic SSE formatı farklı) ──
    if (process.env.ANTHROPIC_API_KEY) {
        try {
            ...
            console.log('[AI] Anthropic sağlayıcısı kullanıldı');
            // Anthropic streaming SSE'yi OpenAI formatına dönüştür
            if (stream) {
                ...
                reader.on('data', chunk => {
                    ...
                    for (const line of lines) {
                        ...
                        try {
                            ...
                            // content_block_delta → OpenAI delta format'ına çevir
                            const token = parsed?.delta?.text || '';
                            if (token) {
                                const openaiChunk = { choices: [{ delta: { content: token } }] };
                ...
                return null; // zaten res'e yazdık
            }
            // Non-stream: parse et, OpenAI formatında döndür
            const d = await r.json();
            const text = d?.content?.[0]?.text || '';
            res.json({ choices: [{ message: { role: 'assistant', content: text } }] });
            return null;
        } catch (e) { console.warn('[AI] Anthropic başarısız, llama.cpp\'ye geçiliyor:', e.message); }
    }

    // ── 4. LLAMA.CPP (yerel) ────────────────────────────────────────
    try {
        const r = await fetch(LLAMA_CPP_URL + '/v1/chat/completions', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ model: process.env.LLAMA_MODEL || 'default', messages, stream: !!stream, max_tokens: maxTok, temperature: temp }),
            signal: AbortSignal.timeout(AI_TIMEOUT_MS)
        });
        if (!r.ok) throw new Error(`llama.cpp HTTP ${r.status}`);
        console.log('[AI] llama.cpp sağlayıcısı kullanıldı');
        return r;
    } catch (e) { console.warn('[AI] llama.cpp başarısız:', e.message); }

    return null; // Hiçbir sağlayıcı yok
}

app.post('/api/ai-chat', authenticateToken, async (req, res) => {
    const { messages, stream = false, model, max_tokens, temperature, lat, lon, city } = req.body;
    ...
    try {
        // ── Hava durumu otomatik enjeksiyon ─────────────────────────
        let enrichedMessages = [...messages];
        if (needsWeather(messages) && process.env.OPENWEATHER_API_KEY) {
            const weather = await fetchWeatherForAI(lat, lon, city);
            if (weather) {
                const weatherInfo = `\n\n[Mevcut hava durumu - ${weather.sehir}: ${weather.sicaklik}°C (hissedilen ${weather.hissedilen}°C), ${weather.durum}, nem %${weather.nem}, rüzgar ${weather.ruzgar} m/s, günlük min/max: ${weather.min}°C/${weather.max}°C]\nBu hava verilerini dikkate alarak tarımsal önerin ver.`;
                // System mesajına ekle (varsa), yoksa başa ekle
                const sysIdx = enrichedMessages.findIndex(m => m.role === 'system');
                if (sysIdx !== -1) {
                    enrichedMessages[sysIdx] = { ...enrichedMessages[sysIdx], content: enrichedMessages[sysIdx].content + weatherInfo };
                } else {
                    enrichedMessages.unshift({ role: 'system', content: 'Sen AgroBot\'sun. Türk çiftçilerine tarım, hayvancılık, ekim, sulama, gübre, hastalık-zararlı konularında Türkçe, kısa ve pratik öneriler veriyorsun.' + weatherInfo });
                }
                console.log(`[AI] Hava durumu enjekte edildi: ${weather.sehir} ${weather.sicaklik}°C`);
            }
        }

        const providerRes = await callAIProvider(enrichedMessages, stream, max_tokens, temperature, res);

        // callAIProvider null döndürdüyse (Anthropic non-stream veya hata)
        if (providerRes === null) {
            if (!res.headersSent) {
                res.status(503).json({ error: 'Hiçbir AI sağlayıcısı çalışmıyor. .env dosyasına GROQ_API_KEY, OPENAI_API_KEY, ANTHROPIC_API_KEY veya LLAMA_CPP_URL ekleyin.' });
            }
            return;
        }

        if (!providerRes.ok) {
            const errTxt = await providerRes.text().catch(() => '');
            return res.status(providerRes.status).json({ error: 'AI hatası: ' + providerRes.status, detail: errTxt.slice(0, 200) });
        }
        ...
        if (stream) {
            ...
            providerRes.body.on('error', () => res.end());
            req.on('close', () => { try { providerRes.body.destroy(); } catch(_) {} });
        } else {
        ...
        if (!res.headersSent) {
            if (error.name === 'AbortError') {
                return res.status(504).json({ error: 'AI zaman aşımı', code: 'AI_TIMEOUT' });
            }
            res.status(503).json({ error: 'AI sunucusuna bağlanılamadı.', code: 'AI_OFFLINE' });
        }
    }
});
...
app.get('/api/ai-chat/health', authenticateToken, async (req, res) => {
    // Herhangi bir sağlayıcı tanımlıysa online say
    const hasGroq      = !!process.env.GROQ_API_KEY;
    const hasOpenAI    = !!process.env.OPENAI_API_KEY;
    const hasAnthropic = !!process.env.ANTHROPIC_API_KEY;
    if (hasGroq || hasOpenAI || hasAnthropic) {
        return res.json({ online: true, provider: hasGroq ? 'groq' : hasOpenAI ? 'openai' : 'anthropic' });
    }
    // llama.cpp kontrol et
    try {
        const { default: fetch } = await import('node-fetch');
        ...
        clearTimeout(t);
        res.json({ online: r.ok, provider: 'llama.cpp', status: r.status });
    } catch {
...
// ── AI yanıt üretici (generateAiResponse) — /api/ai/chat endpointi için ─
// Öncelik: GROQ → OPENAI → ANTHROPIC → llama.cpp
async function generateAiResponse(message, systemPrompt, lang) {
    ...
    if (process.env.GROQ_API_KEY) {
        try {
            const r = await fetch('https://api.groq.com/openai/v1/chat/completions', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${process.env.GROQ_API_KEY}` },
                body: JSON.stringify({ model: process.env.GROQ_MODEL || 'llama-3.3-70b-versatile', max_tokens: parseInt(process.env.AI_MAX_TOKENS) || 1024, temperature: 0.7, messages: msgs }),
                signal: AbortSignal.timeout(parseInt(process.env.AI_TIMEOUT_MS) || 30000)
            });
            if (!r.ok) throw new Error(`Groq HTTP ${r.status}`);
            const d = await r.json();
            const reply = d?.choices?.[0]?.message?.content;
            if (!reply) throw new Error('Groq boş yanıt');
            console.log(`[AI] Groq yanıtı (${reply.length} karakter)`);
            return reply.trim();
        } catch (e) { console.warn('[AI] Groq hatası:', e.message); }
    }
    ...
    if (process.env.OPENAI_API_KEY) {
        try {
            const r = await fetch('https://api.openai.com/v1/chat/completions', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${process.env.OPENAI_API_KEY}` },
                body: JSON.stringify({ model: process.env.OPENAI_MODEL || 'gpt-4o-mini', max_tokens: parseInt(process.env.AI_MAX_TOKENS) || 1024, temperature: 0.7, messages: msgs }),
                signal: AbortSignal.timeout(parseInt(process.env.AI_TIMEOUT_MS) || 30000)
            });
            if (!r.ok) throw new Error(`OpenAI HTTP ${r.status}`);
            const d = await r.json();
            const reply = d?.choices?.[0]?.message?.content;
            if (!reply) throw new Error('OpenAI boş yanıt');
            console.log(`[AI] OpenAI yanıtı (${reply.length} karakter)`);
            return reply.trim();
        } catch (e) { console.warn('[AI] OpenAI hatası:', e.message); }
    }
    ...
    if (process.env.ANTHROPIC_API_KEY) {
        try {
            const r = await fetch('https://api.anthropic.com/v1/messages', {
                ...
                body: JSON.stringify({ model: process.env.ANTHROPIC_MODEL || 'claude-haiku-4-5-20251001', max_tokens: parseInt(process.env.AI_MAX_TOKENS) || 1024, system: systemPrompt, messages: [{ role: 'user', content: message }] }),
                signal: AbortSignal.timeout(parseInt(process.env.AI_TIMEOUT_MS) || 30000)
            });
            if (!r.ok) throw new Error(`Anthropic HTTP ${r.status}`);
            const d = await r.json();
            const reply = d?.content?.[0]?.text;
            if (!reply) throw new Error('Anthropic boş yanıt');
            console.log(`[AI] Anthropic yanıtı (${reply.length} karakter)`);
            return reply.trim();
        } catch (e) { console.warn('[AI] Anthropic hatası:', e.message); }
    }
    ...
    try {
        ...
        const r = await fetch(`${llamaUrl}/v1/chat/completions`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ model: process.env.LLAMA_MODEL || 'default', max_tokens: parseInt(process.env.AI_MAX_TOKENS) || 1024, temperature: 0.7, messages: msgs }),
            signal: AbortSignal.timeout(parseInt(process.env.AI_TIMEOUT_MS) || 60000)
        });
        if (!r.ok) throw new Error(`llama.cpp HTTP ${r.status}`);
        const d = await r.json();
        const reply = d?.choices?.[0]?.message?.content;
        if (!reply) throw new Error('llama.cpp boş yanıt');
        console.log(`[AI] llama.cpp yanıtı (${reply.length} karakter)`);
        return reply.trim();
    } catch (e) { console.warn('[AI] llama.cpp hatası:', e.message); }

    throw new Error('Hiçbir AI sağlayıcısı yapılandırılmamış veya erişilemiyor. .env dosyasına GROQ_API_KEY, OPENAI_API_KEY, ANTHROPIC_API_KEY veya LLAMA_CPP_URL ekleyin.');
}
